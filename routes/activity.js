...
var pgConfig = {
    user: 'ymeczuagvqulez',
    database: 'd5jtorgi61e3i4',
    password: '6b600b33136e8217746298d60540e88c8fc2970ef94d137ce0eb967a87a30f38',
...
"client_id": "a7xyh1ycc23qc39w19p4pgip",
"client_secret": "mnBDFGaRclPqGqysDv7bCLHE" 
};
...
var insertDEUrl = "https://mcjhmstdw76qsk9kk6m1zpwm4xf4.rest.marketingcloudapis.com/data/v1/async/dataextensions/key:TargetDE2/rows";
    ...
    JWT(req.body, 'Hello world', (err, decoded) => {
        ...
        if (decoded && decoded.inArguments && decoded.inArguments.length > 0) {
			...
			for(var i in decoded.inArguments){
				...
				var ID = decoded.inArguments[i].ID;
			...
			if(isEmpty!=true){
				...
				map.status = 'pending';
				var queryStr = 'INSERT INTO ben.input(name,email,startdate,enddate,adcode,journeyid,status,createdate) VALUES($1::varchar, $2::varchar,$3::varchar,$4::varchar,$5::varchar,$6::varchar,$7::varchar,$8::varchar)';
				var parameters = [map.name,map.Email,map.startDate,map.endDate,map.ADCode,map.journeyid,map.status,dateFormat(new Date())];
...
function retrieveDataFromDB(insertDataIntoDE){
	...
	pgPool.connect(function (isErr, client, done) {
        ...
        client.query("select id,name,email,startdate,enddate,adcode from ben.input where status !='success' and journeyid=$1 and createdate <=$2 order by id asc", [journeyID,dateStr], function (isErr, rst) {
            done();//释放连接，归还给连接池
            if (isErr) {
				...
				for(var key in data){
					...
					resultMap.ADCode =data[key].adcode;
